...
public class T01_VectorTest {
	public static void main(String[] args) {
		...
		// List가 가지고 있는 메소드는 Vector가 전부 구현 했다.
		Vector v1 = new Vector(); 
		
		List l1 = new ArrayList();//폴리몰피즘

		System.out.println("처음 크기 : " + v1.size());

		// Vector는 add() 메서드를 이용하여 데이터를 추가할 수 있다.
		v1.add("aaa");
		v1.add(111);// 객체만 담을 수 있음 객체가 아닌 숫자가 들어옴 내부적으로는 new Interger로 객체로 만듬
		v1.add(new Integer(123)); // 기본타입을 Integer라는 객체로 만듬
		v1.add('a');
		v1.add(true);
		v1.add(3.14);
		...
		v1.addElement("ccc"); // add 라는 기능을 addElement를 가지고 있었음 이렇게 해도 되지만 요즘에는 add 씀
		...
		// => 자료가 삭제되면 index 번째 이후의 데이터들이 앞으로 자동으로 당겨져서
		// => 채워진다.
		// => 반환값 : 삭제된 데이터

		// remove(삭제할 데이터) => '삭제할 데이터'를 찾아 삭제한다.
		// => 만약 '삭제할 데이터'가 여러개이면 앞에서부터 삭제한다.
		// => 삭제할 데이터를 객체로 반환해서 사용해야한다.(아니면 인덱스로 인식함)
		...
		/*
		 * 제네릭 타입(generic type) => Collection 객체를 선언할 때 <> 안에 그 Collection이 저장할 데이터 타입을
		 * 정해주는 것을 말한다. => 이런식으로 선언하게 되면 그 데이터 타입 이외이 데이터를 저장할 수 없다. (제너릭 타입으로 선언할 수 있는
		 * 데이터 타입은 클래스여야 한다. (예 : int -> Integer, boolean -> Boolean, char -> Character
		 * 등) => 제너릭 타입으로 선언하게 되면 데이터를 꺼낼 때 별도의 형변환이 필요 없다.
		 */

		Vector<String> v2 = new Vector<String>(); // String만 저장 가능
		Vector<Integer> v3 = new Vector<Integer>(); // String만 저장 가능
		...
		// removeAll(Collection객체) => 벡터의 데티어 중에서 'Collection객체' 가
		// => 가지고 있는 데이터를 모두 삭제한다.
		v2.removeAll(v4);
		...
		/*
		 * 향상된 for문 형식 : for(자료형명 변수명 : 배열변수나 Collection 계열 변수){ 처리할 내용들; ... => 주어진
		 * '배열변수나 Collection 계열 변수'의 데이터 개수 만큼 반복한다. => 반복할 때마다 '배열변수나 Collection 계열변수'의
		 * 데이터를 하낫기 꺼내와 '변수명'에 저장한 후 처리할 내용들'을 처리한다.
		 */
		...
		// 만약 제너릭을 사용하지 않는 Collection을 향상된 for문으로 처리할 경우에는 Object형으로 간주한다.
		for (Object obj : v1) {
		...
		System.out.println("=== After trumTosize() ===");
		print(v);

		v.ensureCapacity(5);// 현재 용량이 최소용량보다 작다면... 신규용량 = 현재용량*2;
							// 그래도 최소용량보다 작다면 신규옹량 = 최소용량으로 설정된다.
		System.out.println("=== After v.ensureCapacity(5) ===");
		print(v);

		v.setSize(7); // 현재 용량이 설정 사이즈보다 작으면 신규옹량 = 현재용량 * 2;
