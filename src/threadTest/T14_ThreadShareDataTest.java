...
class ShareData{
	...
	/* p786
	 	volatile => 선언된 변수를 컴파일러의 최적화 대상에서 제외시킨다.
	 				즉, 값이 변경된느 즉시 변수에 적용시킨다.
	 				다중 쓰레드에서 하나의 변수가 완벽하게한번에 작동하도록
	 				보장하는 키워드(일종의 동기화)
	 */
	volatile public boolean isOk = false; // 원주율이 계산되었는지 나타내는 변수 // volatile 지우면 계속 돌아요
}
...
class CalcPIThread extends Thread{
	...
	public void run() {
		...
		for (int i = 1; i < 2000000000; i+=2) {
			if (((i/2)%2 ==0)) { // 2 로 나눈 몫이 짝수이면 (2로 나누었을때 나머지 0) +
				sum += (1.0/i);
			}else {
				sum -= (1.0/i);
			}
