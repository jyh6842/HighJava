...
public class TcpFileServer {
   ...
   public void serverStart() {
      ...
      try {
         ...
         fin = new FileInputStream(file);  // File file = new File("d:/D_Other/Tulips.jpg"); 이거 가져오려고
         out = socket.getOutputStream();
         
         //한꺼번에 읽어와 전송할 데이터 저장 변수 선언
         byte[] tmp = new byte[1024];
         int length = 0;
         while((length = fin.read(tmp)) != -1) { // 1024byte 까지 차질때까지
            out.write(tmp, 0, length);
         }
         out.flush();// 부족하게 담기 버퍼에 남은거 다 털어버림
