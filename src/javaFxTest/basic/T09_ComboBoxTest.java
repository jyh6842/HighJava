...
public class T09_ComboBoxTest extends Application {
	...
	public void start(Stage primaryStage) throws Exception {
		...
		combo.setValue("한강"); // 해놓지 않으면 처음 선택이 안되어 있다.
		...
		combo.valueProperty().addListener(new ChangeListener<String>() {
			...
			public void changed( // 바꿧을때 값이 적용된다?
				...
				txtArea.setText(newValue); // 이거 지우면 textArea에 지역 고를때마다 안써 있음 확인 눌러야 써짐
...
//		ComboBox<String> combo2 = new ComboBox<>(fruitList);// 이렇게 하나로도 되고
	      ComboBox<String> combo2 = new ComboBox<>(); // combo2.getItems().addAll(fruitList); //  combo2.setItems(fruitList); 둘 중 하나 쓰면 됨
		...
		button.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				if(combo.getValue() != null && combo2.getValue() != null) {
					txtArea.setText(combo.getValue()+"지역에는 " //setText는 덮어 씌우는 것이다. txtArea.appendText() 는 추가하는 것이고
