...
public class T15_TableViewTest extends Application {
	...
	public void start(Stage primaryStage) throws Exception {
		...
		ObservableList<Member> data = FXCollections.observableArrayList(
				new Member("홍길동", "gildong", 22, "2222-2222", "대전"), new Member("홍길서", "gilseo", 33, "3333-3333", "서울"),
				new Member("홍길남", "gilnam", 44, "4444-4444", "부산"),
				new Member("홍길북", "gilbook", 55, "5555-5555", "광주"));

		BorderPane root = new BorderPane();

		// TableView 에 데이털를 셋팅하기 => 방법1(TableView의 생성자 이용)
		TableView<Member> table = new TableView<>(data);

		TableColumn<Member, String> nameCol = new TableColumn<Member, String>("이름"); // 제너릭이 2개로 되어있는 클래스
		TableColumn<Member, String> kornameCol = new TableColumn<Member, String>("한글이름");
		kornameCol.setStyle("-fx-alignment : CENTER"); // 중앙 정렬

		// 해당 컬럼에 나타낼 데이터 연결하기
		kornameCol.setCellValueFactory(new PropertyValueFactory<>("KorName")); // setCellValueFactory 이걸하면 어떤 데이터를 보여줘야
																				// 할지 모르기 때문에 값이 안보일 것임. 실제 데이터 값을 만들어주는
																				// 메소드
		...
		// 생성된 각 컬럼들을 TableView 에 추가한다.

		table.getColumns().addAll(nameCol, ageCol, telCol, addrCol); // 병합했을때 이렇게 넣습니다.
//		table.getColumns().addAll(kornameCol , engNameCol, ageCol, telCol, addrCol); // 병합하지 않으면

		// TableView에 데이터를 셋팅하기 => 셋팅하기 방법2 (setItems() 메서드 이용)
		table.setItems(data);
		// ------------------------------------------------------------------------------
		...
		btnEdit.setOnAction(e -> {
			...
			data.set(table.getSelectionModel().getSelectedIndex(), // 인덱스번호 찾기 //.getSelectedItem : vo 찾는거
					new Member(txtKorName.getText(), // 수정할 데이터
							txtEngName.getText(), Integer.parseInt(txtAge.getText()), txtTel.getText(),
							txtAddr.getText()));
