package rmi.java_rmi.basic.client.client;
...
import rmi.java_rmi.basic.inf.RemoteInterface;
import rmi.java_rmi.basic.vo.FileInfoVO;
import rmi.java_rmi.basic.vo.TestVO;
...
public class RemoteClient {
   public static void main(String[] args) {
      ...
      try {
         ...
         //  사용할 객체를 불러온다.
         Registry reg = LocateRegistry.getRegistry("localhost", 8429);
         ...
         files[1] = new File("d:/D_Other/Desert.jpg");
         
         FileInfoVO[] fInfo = new FileInfoVO[files.length];
         
         //2개의 파일을 읽어서 byte[]에 담아서 서버측 메서드에
         //전달하면 된다.
         ...
         for(int i = 0; i < files.length; i++) {
            ...
            fInfo[i].setFileName(files[i].getName());   //파일 명 저장
            fInfo[i].setFileData(data);   //파일데이터 저장         
         }
         
         clientInf.setFiles(fInfo);   //서버의 파일 저장메서드 호출
