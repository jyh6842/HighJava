...
public class T07_WildCardTest {
	public static void main(String[] args) {
		...
		//Juicer.makeJuice(fruitBox);  // 과일상자인 경우에는 아무런 문제 없음 // 1번 짝꿍
		//Juicer.makeJuice(appleBox); // 와일드카드를 이용한 지네릭타입의 확장으로 에러 해결가능 //2번짝꿍 3번짝꿍도 된다.
 ...
 */
class Juicer{
	//static void makeJuice(FruitBox<Fruit> box) { // 제너릭 타입 객체를 파라미터에 사용시 문제점 발생함. // 1번 짝꿍 // Fruit만 올 수 있다. 사과도 오고 싶고 포도도 오고 싶은데 올 수 없다.
		                                         // 지네릭 타입이 다른 것만으로는 오버로딩이 성립하지 않음 => 컴파일 후 제거됨.=> 메서드 중복정의
		                                         // 와일드 카드를 이용하여 해결가능.
    
	static void makeJuice(FruitBox<? extends Fruit> box) {  // 와일드카드를 이용하여 제너릭 타입의 객체 참조 // 2번 짝꿍 
	//static <T extends Fruit> void makeJuice(FruitBox<T> box) { // 지네릭 메서드(제한된 타입 파라미터)로 선언함. 3번 짝꿍
