...
/*
 	...
 	프로그램 소스코드 안에 다른 프로그램을 윟나 정보를 미리 약속된 형식으로 포함 시킨것 (JDK1.5부터 지원함
 	...
 	애너테이션 타입 정의하기
 	@interface 애너테이션이름 {
 		타입요소이름(); // 반환값이 잇고 매개변수는 없는 추상 메서드의 형태
 		...
 	}
 	...
 	2. () 안에 매개변수를 선언할 수 없다.
 	3. 예외를 선언할 수 없다.
 	4. 요소의 타입에 타입 매개변수(지네릭타입문자)를 사용할수 없다.
 	
 */
@Target(ElementType.METHOD) //annotation 이 적용가능한 대상을 지정함
@Retention(RetentionPolicy.RUNTIME) // annotation이 유지되는 기간(SOURCE, CLASS, RUNTIME)
public @interface PrintAnnotation {
	
	int id = 100; // 상수 선언 가능. static final int id = 100; // 따로 static final로 쓸 필요 없이 저렇게 넘어감
